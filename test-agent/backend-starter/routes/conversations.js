...
router.post('/:id/messages', async (req, res, next) => {
  try {
    ...
    // Simulate AI Agent auto-reply if the AI has not been fully stopped, 
    // or just to provide immediate test feedback
    setTimeout(async () => {
      try {
        const activeConv = await Conversation.findOne({ id: req.params.id });
        if (activeConv) {
          const aiReply = {
            sender: 'agent',
            text: "I received your message! As an AI agent simulations, I am here to help. Is there anything specific you'd like me to look into?",
            timestamp: new Date()
          };
          activeConv.messages.push(aiReply);
          await activeConv.save();
          
          // Broadcast the agent auto-reply via WebSocket
